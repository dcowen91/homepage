...
<body>
  ...
  function showBlog(message) {
    ...
    $('#blog').html('<h1 class="margin-base-vertical">' + message['title'] + '</h1> <p>' + message['content'] + '</p>' + '<small> <em>' + posted on ' + posted.toLocaleDateString() + ' at ' + posted.toLocaleTimeString() + '</small </em>'+ '<button id="leftPost" type="button" class="btn btn-default btn-primary">Next</button> <button id="rightPost" type="button" class="btn btn-default btn-primary">Prev</button>');
  }
